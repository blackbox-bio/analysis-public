...
def extract_posture_with_moving_background(recording_name, trans_path, ftir_path, dlc_path):

    print(f"Processing {recording_name}...")

    fourcc = cv2.VideoWriter_fourcc(*'H264')
    output_trans_path = os.path.join(os.path.dirname(trans_path), "trans_posture_with_bg.mp4")
    output_ftir_path = os.path.join(os.path.dirname(ftir_path), "ftir_posture_with_bg.mp4")
    ...
    speed_along_orientation = np.einsum('ij,ij->i', displacement, orientation_vector[:-1])  # Dot product
    speed_along_orientation = np.insert(speed_along_orientation, 0, 0)  # Insert 0 for the first frame

    # Calculate speed perpendicular to the orientation vector
    perpendicular_velocity = np.einsum('ij,ij->i', displacement, np.flip(orientation_vector[:-1], axis=1) * [-1, 1])
    perpendicular_velocity = np.insert(perpendicular_velocity, 0, 0)  # Insert 0 for the first frame
    ...
    if frame_num > orientation_vector.shape[0]:
        ...
        print("Warning: The orientation vector is shorter than the video, which means the last few frames in the video were corrupted and will not be processed.")
    ...
    for y in range(0, frame_ht, stripe_spacing):
        bg_pattern_horizontal[y:y+stripe_thickness, :left_edge_width] = (128, 128, 128)
        bg_pattern_horizontal[y:y+stripe_thickness, right_edge_start:] = (128, 128, 128)
    ...
    for x in range(0, frame_wd, stripe_spacing):
        ...
        bg_pattern_vertical[bottom_bar_start:, x:x+stripe_thickness] = (128, 128, 128)
    ...
    for i in tqdm(range(frame_num)):
        ...
        bg_offset_horizontal = (bg_offset_horizontal - int(speed_along_orientation[i])) % frame_ht
        bg_offset_vertical = (bg_offset_vertical + int(perpendicular_velocity[i])) % frame_wd

        # Create moving backgrounds
        moving_bg_horizontal = np.roll(bg_pattern_horizontal, -bg_offset_horizontal, axis=0)  # Roll vertically
        moving_bg_vertical = np.roll(bg_pattern_vertical, -bg_offset_vertical, axis=1)        # Roll horizontally
        ...
        blended_frame_trans = cv2.addWeighted(combined_bg, 0.5, frame_trans_transformed, 1, 0)
        blended_frame_ftir = cv2.addWeighted(combined_bg, 0.5, frame_ftir_transformed, 1, 0)
