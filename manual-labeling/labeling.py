...
def generate_empty_score_table():
    ...
    return pd.DataFrame(
        np.zeros((total_frames, len(behaviors))).astype(int), columns=behaviors
    )
    ...
    while frame_index < total_frames:
        ...
        if total_frames - frame_index >= 200:
            segment_length = np.random.randint(
                200, min(500, total_frames - frame_index)
            )
        else:
...
def get_behaviors_for_frame():
    ...
    labeled_behaviors = score_table.columns[
        score_table.iloc[current_frame] == 1
    ].tolist()
...
def plot_score_table():
    ...
    if video_path is None:
        ...
        ax.imshow(
            np.zeros((total_frames, len(behaviors))).T,
            aspect="auto",
            cmap="Greys",
            interpolation="none",
        )
...
def export_csv():
    ...
    output.headers[
        "Content-Disposition"
    ] = "attachment; filename=behavior_score_table.csv"
    return output
